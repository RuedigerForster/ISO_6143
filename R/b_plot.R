...
b_plot <- function(cal_data, meas_data, b, b_cov, func) {
  ...
  arrows(cal_data[, 1], cal_data[, 3] - k * cal_data[, 4],
         ...
         angle = 90, code = 3, length = 0.05, col = "red")
  points(cal_data[, 1], cal_data[, 3], pch = 16, col = "red")
  # Measurement points
  for (i in seq_len(nrow(meas_data))) {
    cv_i <- matrix(c(xc_meas[i, i], xyc[i, i], xyc[i, i], yc_meas[i, i]), 2, 2)
    .b_plot_ellipse(x_meas[i], meas_data[i, 1], cv_i, "black")
  }
  ...
  arrows(x_meas, meas_data[, 1] - k * sqrt(diag(yc_meas)),
         ...
         angle = 90, code = 3, length = 0.05, col = "black")
  points(x_meas, meas_data[, 1], pch = 16, col = "black")
  legend("topleft",
